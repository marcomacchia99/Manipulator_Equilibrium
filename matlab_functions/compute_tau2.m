function tau2=compute_tau2(angle2,g,force_point1,force_point2,force_point3,moment_point1,moment_point2,moment_point3)
...
CoM1 = CoM_link1*smiData.Solid(3).mass/(smiData.Solid(2).mass+smiData.Solid(3).mass)*[cos(deg2rad(0)) sin(deg2rad(0)) 0]/1000;
...
%second motor
%angle1 is set to 0 because it is a prismatic joint
CoM2 = (CoM_motor2*[cos(deg2rad(0)) sin(deg2rad(0)) 0]+(CoM_link2-CoM_motor2)*smiData.Solid(5).mass/(smiData.Solid(5).mass+smiData.Solid(4).mass)*[cos(deg2rad(angle2)) sin(deg2rad(angle2)) 0])/1000;

%% define application points

%compute the three wrench application points
point1 = ([CoM_motor2 0 0]+(norm((smiData.RigidTransform(5).translation-smiData.RigidTransform(4).translation))*[cos(deg2rad(angle2)) sin(deg2rad(angle2)) 0]))/1000;
point2 = [CoM2(1)+0.15*cos(deg2rad(angle2)) CoM2(2)+0.15*sin(deg2rad(angle2)) CoM2(3)];
point3 = [CoM_motor2-20 0 0]/1000;

%% compute tranformation matrices

%compute the origins of the two joints
origin1 = zeros(3,1);
origin2 = norm(smiData.RigidTransform(4).translation)*[cos(deg2rad(0)) sin(deg2rad(0)) 0]/1000;

T1 = transf_mat(0,origin1);
T2 = transf_mat(angle2,origin2);
...
C = J2(4:6,:)'*(smiData.Solid(5).mass+smiData.Solid(4).mass)*0.453592*g';

%% compute final

tau2 = -(J1'*S_3_1'*W3)-(J2'*S_1_2'*W1)-(J2'*S_2_2'*W2)-C;
